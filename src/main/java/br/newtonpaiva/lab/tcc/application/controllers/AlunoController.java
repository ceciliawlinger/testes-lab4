...
public class AlunoController implements AlunosResource {
    ...
    public ResponseEntity<AlunoResponse> create(@Valid Optional<AlunoRequest> request) {
        ...
        Aluno aluno = new Aluno().withAnoSemestreDeEntrada(request.get().getAnoSemestreDeEntrada()).withCurso(curso)
                .withMatricula(request.get().getMatricula()).withNomeCompleto(request.get().getNomeCompleto());
