...
public class ProfessorController implements ProfessoresResource {
    ...
    ProfessorController(ProfessorService professorService, CursoService cursoService) {
    ...
    public ResponseEntity<ProfessorResponse> create(@Valid Optional<ProfessorRequest> request) {
        ...
        Professor professor = new Professor().withCurso(curso).withMatricula(request.get().getMatricula())
                .withNomeCompleto(request.get().getNomeCompleto()).withFormação(request.get().getFormação());
